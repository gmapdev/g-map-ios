...
/// This is used to make the network request for APIs
open class IBIRequest: NSObject, URLSessionDelegate {
	...
	public enum ParameterFormat: String
	{
		...
		/// This is the plain/text parameter
		case PlainText = "PlainText"
	}
	
	/// Pre-defined header fields, we can put the predefined values, in this place.
	#if !targetEnvironment(macCatalyst)
	public var globalHeaders = ["device_os":"iOS_\(UIDevice.current.systemVersion)"]
	#else
	public var globalHeaders = ["device_os":"iOS_Mac_\(UIDevice.current.systemVersion)"]
	#endif
 ...
 ///   - timeout: Parameter description
 ///   - callback: Parameter description
	public func request(method: RequestMethod, path:String, params: String?, headers: [String: String]?, format:ParameterFormat = ParameterFormat.PlainText, queue: DispatchQueue = DispatchQueue.global(), timeout:Double? = nil, callback: @escaping APICallback) {
		let _ :String? = self.requestRestAPI(method: method, path: path, params: params, headers: headers, format:format, queue: queue, timeout: timeout) { (data, error, response) in
			callback(data, error, response)
		}
	}
 ...
 ///   - callback: Parameter description
	public func multipartRequest(method: RequestMethod, path: String, fileName: String?, file: Data?, params:[String: Any]?, headers: [String: String]?, queue: DispatchQueue = DispatchQueue.global(), timeout: Double? = nil, callback: @escaping APICallback){
		...
		guard let url = URL(string: path) else {
			...
			callback(nil, IBIError.notValidURL, nil)
			return
		}
				...
				if let httpResponse = (response as? HTTPURLResponse) {
					...
					if status < 400 && status >= 200 {
						if let data = data {
							...
							OTPLog.log(level: .error, info: "multipart/form-data; response is empty, status code: \(status), url: \(path)")
							callback(nil, IBIError.responseIsEmpty, response)
						}
					}else{
						OTPLog.log(level: .error, info: "multipart/form-data; request failed, status code: \(status), url: \(path)")
						callback(data, IBIError.requestFailed, response)
					}
				}else{
					OTPLog.log(level: .error, info: "multipart/form-data; Unknown response type, url: \(path)")
					callback(nil, IBIError.unknownResponseType, response)
				}
			}
			dataTask.resume()
		}
		...
		{
			OTPLog.log(level: .error, info: "multipart/form-data; Url is not initialized. url: \(path)")
			callback(nil, IBIError.urlSessionIsNotInitialized, nil)
		}
	}
	...
	/**
	 ...
	 - parameter params: the parameters that we want to pass to the request. either [String], or [String: Any] or [[String:Any]] or String
	...
	private func requestRestAPI<T>(method: RequestMethod, path:String, params: T?, headers: [String: String]?, format:ParameterFormat = ParameterFormat.PathParameter, queue: DispatchQueue = DispatchQueue.global(), timeout:Double? = nil, callback: @escaping APICallback) -> T? {
		...
		switch(format){
			...
			case .PlainText:
				...
				if let dataString = params as? String {
					bodyData = dataString.data(using: .utf8)
				}
				break
		}
				...
				if let httpResponse = (response as? HTTPURLResponse) {
					...
					if status < 400 && status >= 200 {
						if let data = data {
							...
							OTPLog.log(level: .error, info: "response is empty, status code: \(status), url: \(path)")
							callback(nil, IBIError.responseIsEmpty, response)
						}
					}else{
						OTPLog.log(level: .error, info: "request failed, status code: \(status), url: \(path)")
						callback(data, IBIError.requestFailed, response)
					}
				}else{
					OTPLog.log(level: .error, info: "Unknown response type, url: \(path)")
					callback(nil, IBIError.unknownResponseType, response)
				}
			}
			dataTask.resume()
		...
		{
			OTPLog.log(level: .error, info: "Url is not initialized., url: \(path)")
			callback(nil, IBIError.urlSessionIsNotInitialized, nil)
		}
		return nil
	}
