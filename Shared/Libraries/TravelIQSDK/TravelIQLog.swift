...
/// Travel IQ Log item, which is used to store the log in a structured way
fileprivate struct TravelIQLogItem: Hashable, Codable
{
	/// Define the log level of this item
	var logLevel: TravelIQLog.Level
	...
	var  prettyLogDateTime: String {
		get {
			return TravelIQUtils.convertTimestampToLocal(logDateTime, toFormat: "yyyy-MM-dd HH:mm:ss.SSSS")
		}
	}
}
@objc public class TravelIQLog: NSObject {
	
	// MARK: PRIVATE DEFINITION
	
	/// This is the container to save all the log information
	private var logContainer:[TravelIQLogItem] = [TravelIQLogItem]()
	
	/// This is the lock for the log, so that we won't have issue, when we log information in multithread programming
	private var logContainerLock: DispatchQueue = DispatchQueue(label: "com.ibigroup.TravelIQLog.acces.log.queue")
	...
	public var logFileName = "tav7_log_v2.txt"
	...
	/// Shared Instance - TravelIQ Log information
    public static let shared : TravelIQLog = {
        let mgr = TravelIQLog()
	...
	/**
	 ...
	 TravelIQLog won't be recorded. The tracking level priority is info > warning > error > crash
	*/
	...
	@objc public static var flushLogAfterNrofRecords: Int {
		...
		set {
			if newValue <= TravelIQLog.trackingBufferSize && newValue >= 5 {
				shared._flushLogAfterNrofRecords = newValue
			}else {
				print("failed to setup flushLogAfterNrofRecords variable, `flush log after number of records` can not be less than 5 records or larger than tracking buffer size")
			}
		}
	}
    
    /// This property is used to turn on/off the ability of the TravelIQLog to display/print the log in the console. if this is false, there is no log will be printed in the console.
 ...
 /// - Returns: Void)? = nil)
	@objc public static func removeLogFile(completion:(()->Void)? = nil){
		let urlFromDisk = TravelIQUtils.docPath(shared.logFileName)
		if FileManager.default.fileExists(atPath: urlFromDisk.path) {
			shared.logContainerLock.sync {
				do{
					...
					assert(false, "Can not delete local log file for TravelIQ, error:" + error.localizedDescription)
				}
			}
		}else{
			completion?()
    ...
    @objc public static func retrieveLogHistory(withDetails:Bool = false, completion:@escaping (String?)->Void){
		shared.logContainerLock.sync {
			...
			for item in copyLogContainer {
                logHistory.append(shared.formatTravelIQLogItem(item: item, withDetails: withDetails))
			}
 ...
 ///   - lineNumber: Int = #line
	internal static func track<T>(level:Level = .info, info: String, parameters: T? = nil, fileName: String = #file, functionName: String = #function, lineNumber: Int = #line){
		...
		shared.logContainerLock.sync {
			...
			if shared.logContainer.count > TravelIQLog.trackingBufferSize {
				shared.logContainer.removeFirst()
			}
			
            var parameterString : String?
			if let params = parameters{
				parameterString = TravelIQUtils.objToJSONString(object: params) ?? ""
			}
			
			let logEntry = TravelIQLogItem(logLevel: level, logDateTime: Date().timeIntervalSince1970, logMessage: info, fileName: fileName, functionName: functionName, line: lineNumber, parameters: parameterString)

            // Output the log information
            if shared._displayLogInConsole {
                print(shared.formatTravelIQLogItem(item: logEntry, withDetails: shared._displayLogDetails))
            }
    ...
    /// This is used to format a given TravelIQLog Item in a string for console / log display purpose
    /// Formats travel iq log item.
    /// - Parameters:
    ///   - item: TravelIQLogItem
    ///   - withDetails: Bool = false
    /// - Returns: String
    fileprivate func formatTravelIQLogItem(item: TravelIQLogItem, withDetails: Bool = false)->String {
        var outputFormat = ""
        if withDetails {
 ...
 ///   - lineNumber: Int = #line
	fileprivate func flushLogToDisk(fileName: String = #file, functionName: String = #function, lineNumber: Int = #line){
		let urlToDisk = TravelIQUtils.docPath(logFileName)
		if let jsonData = try? JSONEncoder().encode(logContainer) {
			...
			do{
				...
				assert(false, error)
				let logEntry = TravelIQLogItem(logLevel: .error, logDateTime: Date().timeIntervalSince1970, logMessage: error, fileName: fileName, functionName: functionName, line: lineNumber, parameters: "JSON:\(jsonString)")
 ...
 ///   - lineNumber: Int = #line
	fileprivate func loadLogToContainer(fileName: String = #file, functionName: String = #function, lineNumber: Int = #line){
		logContainerLock.sync {
			let urlFromDisk = TravelIQUtils.docPath(self.logFileName)
			if FileManager.default.fileExists(atPath: urlFromDisk.path) {
     ...
     ///   - encoding: .utf8
					...
					if let jsonData = jsonString?.data(using: .utf8) {
						if let content:[TravelIQLogItem] = try? JSONDecoder().decode([TravelIQLogItem].self, from: jsonData){
							...
							let logEntry = TravelIQLogItem(logLevel: .error, logDateTime: Date().timeIntervalSince1970, logMessage: error, fileName: fileName, functionName: functionName, line: lineNumber, parameters: jsonString)
						...
						let logEntry = TravelIQLogItem(logLevel: .error, logDateTime: Date().timeIntervalSince1970, logMessage: error, fileName: fileName, functionName: functionName, line: lineNumber, parameters: "")
						self.logContainer.append(logEntry)
						self.flushLogIntervalIndex += 1
						assert(false, error)
					}
				...
				let logEntry = TravelIQLogItem(logLevel: .warning, logDateTime: Date().timeIntervalSince1970, logMessage: warning, fileName: fileName, functionName: functionName, line: lineNumber, parameters: "")
				self.logContainer.append(logEntry)
				self.flushLogIntervalIndex += 1
			}
		}
